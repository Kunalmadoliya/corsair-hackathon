...
import { useSignup, useSupportedProviders } from "~/hooks/api/auth/auth";
...
export function SignupForm() {
  ...
  const { createUserEmailAndPasswordAsync, isPending, isError, error } = useSignup();
  ...
  return (
    <Card className="w-full max-w-md border-border/40 bg-card/60 backdrop-blur-xl shadow-2xl animate-fade-in relative z-10 overflow-hidden">
      ...
      <div className="text-center space-y-1 pb-2 pt-8 px-8">
        <div className="mx-auto w-12 h-12 rounded-2xl bg-primary/10 border border-primary/20 flex items-center justify-center mb-4 animate-float shadow-[0_0_20px_rgba(139,92,246,0.1)]">
      ...
      <form onSubmit={handleSubmit} className="p-8 pt-4 space-y-6">
        ...
        <div className="space-y-4 pt-2">
          <Button
            type="submit"
            className="w-full h-11 bg-primary text-primary-foreground hover:bg-primary/95 btn-hover font-medium shadow-[0_0_15px_rgba(139,92,246,0.2)]"
            disabled={isPending}
          >
...
function CardTitleAndDescription() {
  return (
    <>
      ...
      <p className="text-muted-foreground/80 text-base">
        Join Spamurai and automate your workflows
      </p>
    </>
  );
}
