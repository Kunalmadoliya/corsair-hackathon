...
export function DemoConversation() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setActiveIdx((p) => (p + 1) % demoPrompts.length), 2000);
        ...
        <div className="flex items-start gap-3">
          ...
          <p className="text-sm text-foreground/90 pt-0.5 leading-relaxed">
            {phase === "typing" ? (
              <>
                {typedText}
                <span className="inline-block w-[2px] h-4 bg-primary ml-0.5 animate-blink align-middle" />
              </>
            ) : (
              current.prompt
            )}
          ...
          <div className="flex items-start gap-3 animate-fade-in">
            ...
            <div className="flex-1 space-y-2 pt-0.5">
              {current.responses.map((resp, i) => (
                <div
                  key={i}
                  className={`flex items-center gap-2 transition-all duration-300 ${visibleResponses.includes(i) ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"}`}
                >
